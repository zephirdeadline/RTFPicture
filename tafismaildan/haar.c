...
/*void cascade(example* arr, example* arr2, example* arr3, float fpr, int l, int gl)
{
  float nfpr = 1;
  int ngl = 0;
  while(nfpr > fpr)
  {
    ...
    int tl = 1;
		...
		int bg = 0;
		int nbl = 0;
		eight : adaboost(arr, tl, 796);*/
		/*ecrire dans cascade.txt, le contenu de strongclassifieur.txt*/
		/*copy();
		//ecrire la fonction qui evalue un classifieur fort
		//et faire l'evaluation pour les 3 tableux
		nine : //faire une boucle jusqua 298 qui compte le nombre d'image ou ya une erreur (gamma e)
		...
		{
			if(((ngl <= gl) && (1-nbl < 1-bl)) && (u > 0.00005 ))
			{
				snl += u;
				if((snl<-1) || (snl>1))
				{
